...
export const Pruebas: React.FC<PruebasProps> = ({ level }) => {
  ...
  const { data: tests = [] as Test[], isLoading: loadingTests } = useTests(
  ...
  const onSubmit = async (data: TestFormValues) => {
    if (
      !createMutationGuard(createTestM.isPending, () =>
        showToast({
          type: TOAST_TYPES.WARNING,
          message: 'Ya se está procesando una creación de prueba',
        })
      )
    ) {
      return
    ...
    try {
      ...
      await queryClient.invalidateQueries({
        queryKey: QUERY_KEYS_INVALIDATE.TESTS,
      })
    } catch (error) {
      console.error(error)
      showToast({
  ...
  return (
    <div className="space-y-10">
        ...
        filters={
          <>
            <div className="flex items-center gap-4">
              ...
              <div className="text-2xl font-bold text-slate-900 min-w-[150px] text-center capitalize">
            ...
            <Select
              aria-label="Filtrar evaluaciones por curso"
      ...
      {courses.length === 0 && !loading && (
        <div className="max-w-4xl mx-auto">
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 flex items-center gap-4 text-amber-800">
            ...
            <div>
              <p className="font-bold">
                No se encontraron cursos para el nivel{' '}
                {level === 'BASICA' ? 'Básica' : 'Media'}.
              </p>
              <p className="text-sm">
                Por favor, verifique la configuración o cargue los datos de
                demostración en la sección de Configuración.
              </p>
            </div>
            {loadingTests && (
              <div className="absolute inset-0 bg-white/70 rounded-[2.5rem] flex items-center justify-center z-40">
                <div className="text-center">
                  <svg
                    className="animate-spin h-10 w-10 mx-auto text-indigo-600"
                    viewBox="0 0 24 24"
                  >
                    <circle
      ...
      <Modal
        testId="modal-create-test"
        isOpen={isModalOpen}
        onClose={() => patchUiState({ isModalOpen: false })}
        title="Programar Nueva Prueba"
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Select
                data-testid="create-test-course"
                label="Curso"
                options={courseOptions}
                {...register('course_id', { required: 'Curso requerido' })}
              />
          ...
          <div>
            <Input
              ...
              label="Asignatura"
              {...register('subject', { required: 'Asignatura requerida' })}
            />
          ...
          <div className="space-y-2">
            ...
            <label
              htmlFor="create-test-description"
              className="text-sm font-bold text-slate-700 ml-1"
            >
              Descripción / Contenidos (Opcional)
            </label>
            <textarea
              id="create-test-description"
              data-testid="create-test-desc"
              {...register('description')}
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 min-h-[100px]"
          ...
          <div className="flex justify-end gap-3 pt-4">
            <Button
              variant="ghost"
              data-testid="create-test-cancel"
              onClick={() => patchUiState({ isModalOpen: false })}
            >
              Cancelar
            </Button>
            <Button
