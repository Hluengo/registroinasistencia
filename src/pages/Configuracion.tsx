...
export const Configuracion: React.FC = () => {
  ...
  const handleSeed = async () => {
    if (
      !confirm(
        '¿Desea cargar datos de prueba? Esto agregará cursos, estudiantes y pruebas de ejemplo.'
      )
    )
      return
    try {
      ...
      showToast({
        ...
        message:
          'Datos cargados exitosamente. Recargue la página para ver los cambios.',
        duration: 5000,
      })
    } catch (error: unknown) {
      ...
      showToast({
        ...
        message:
          ...
          (error instanceof Error
            ? error.message
            : 'Verifique la conexión con Supabase y que las tablas existan.'),
        duration: 5000,
      })
    } finally {
      setLoading(false)
    ...
    Papa.parse(file, {
      ...
      complete: async (results: Papa.ParseResult<Record<string, string>>) => {
        try {
          ...
          const coursesWithLevel = data.map((c) => ({
            ...c,
            level: uploadLevel,
          }))
          ...
          showToast({
            type: TOAST_TYPES.SUCCESS,
            message: 'Cursos cargados exitosamente',
          })
          coursesQ.refetch()
        } catch (error: unknown) {
          showToast({
      ...
      complete: async (results: Papa.ParseResult<Record<string, string>>) => {
        try {
          ...
          const allCourses = coursesQ.data
            ? coursesQ.data.filter((c) => c.level === uploadLevel)
            : []
          const studentsToInsert = data.map((s) => {
            ...
            if (!course)
              throw new Error(
                `Curso no encontrado en ${uploadLevel}: ${s.course_name}`
              )
            return {
          ...
          showToast({
            type: TOAST_TYPES.SUCCESS,
            message: 'Estudiantes cargados exitosamente',
          })
        } catch (error: unknown) {
          showToast({
            type: TOAST_TYPES.ERROR,
        ...
        <div className="bg-white p-10 rounded-3xl border border-slate-200/60 shadow-sm shadow-slate-200/20">
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-slate-500 text-sm font-medium">
                  Administración inicial y carga de catálogos.
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="p-8 bg-slate-50/50 rounded-3xl border border-slate-100 flex flex-col justify-between group hover:bg-slate-50 transition-colors">
              <div>
                <h4 className="font-bold text-slate-900 mb-2 tracking-tight">
                  Carga Masiva de Cursos
                </h4>
                <p className="text-sm text-slate-500 mb-8 font-medium">
                  Suba un archivo CSV con la columna{' '}
                  <code className="bg-slate-200/50 px-1.5 py-0.5 rounded text-indigo-600 font-mono text-xs">
                    name
                  </code>
                  .
                </p>
              ...
              <div className="relative">
                ...
                <button
                  type="button"
                  className="w-full flex items-center justify-center gap-3 bg-slate-900 text-white px-8 py-4 rounded-2xl font-bold hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/10"
                >
            ...
            <div className="p-8 bg-slate-50/50 rounded-3xl border border-slate-100 flex flex-col justify-between group hover:bg-slate-50 transition-colors">
              ...
              <div className="relative">
                ...
                <button
                  type="button"
                  className="w-full flex items-center justify-center gap-3 bg-indigo-600 text-white px-8 py-4 rounded-2xl font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-600/20"
                >
            ...
            <div className="p-8 bg-amber-50/50 rounded-3xl border border-amber-100/50 flex flex-col md:flex-row md:items-center justify-between gap-8">
              <div className="flex-1">
                ...
                <p className="text-sm text-amber-800/70 leading-relaxed font-medium">
                  Genera automáticamente cursos, estudiantes y un calendario de
                  pruebas para probar las funcionalidades del sistema.
                </p>
              </div>
              <button
                type="button"
