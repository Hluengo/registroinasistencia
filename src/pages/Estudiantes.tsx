...
import { Search, Calendar, ShieldAlert, ChevronRight } from 'lucide-react'
...
export const Estudiantes: React.FC<EstudiantesProps> = ({ level }) => {
  ...
  const { data: studentsFromQuery = [], isLoading: studentsLoading } =
    useStudents(selectedCourse || undefined, level)
  const { data: coursesFromQuery = [], isLoading: coursesLoading } =
    useCourses(level)
  ...
  const {
    data: studentDetailsFromQuery = { absences: [], records: [] },
    isLoading: studentDetailsLoading,
  } = useStudentDetails(selectedStudent?.id || undefined, isModalOpen)
  ...
  return (
    <div className="space-y-10">
          ...
          <>
            <div className="relative w-full md:w-96">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/10 transition-colors"
              />
            </div>
            <Select
            ...
            {(selectedCourse || searchQuery) && (
              <Button
                ...
                size="sm"
                onClick={() => {
                  patchUiState({ selectedCourse: '', searchQuery: '' })
                }}
                className="font-bold text-slate-400 hover:text-indigo-600"
              >
                Limpiar
              </Button>
            )}
      ...
      <StudentDetailModal
        ...
        setActiveTab={(activeTab) => patchUiState({ activeTab })}
        studentDetails={studentDetailsFromQuery}
    ...
    <tr className="hover:bg-slate-50/80 transition-colors group">
      ...
      <td className="px-6 py-5">
        <span className="text-sm font-semibold text-slate-600 bg-slate-100/50 px-2 py-1 rounded-lg border border-slate-200/50">
          {courses.find((c) => c.id === student.course_id)?.name || 'N/A'}
        </span>
      </td>
...
function StudentDetailModal({
  ...
  onClose: () => void
  selectedStudent: Student | null
  setActiveTab: (tab: 'absences' | 'records') => void
  studentDetails: { absences: Absence[]; records: InspectorateRecord[] }
      ...
      <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
              <Badge variant="secondary">
                {courses.find((c) => c.id === selectedStudent?.course_id)
                  ?.name || 'N/A'}
              </Badge>
              <span className="text-slate-400">•</span>
              <span className="text-slate-500 font-medium">
                RUT: {selectedStudent?.rut || 'N/A'}
              </span>
            </div>
          </div>
        </div>

        <StudentDetailTabs activeTab={activeTab} setActiveTab={setActiveTab} />

        <div
          className="min-h-[300px] py-2"
          role="tabpanel"
          id={
            activeTab === 'absences' ? 'tabpanel-absences' : 'tabpanel-records'
          }
          aria-labelledby={
        ...
        >
          {studentDetailsLoading ? (
            ...
            <StudentAbsencesList absences={studentDetails.absences} />
          ) : (
            <StudentRecordsList records={studentDetails.records} />
          )}
        </div>
      </div>
    </Modal>
  ...
  return (
    ...
    >
      <button
        ...
        className={cn(
          'px-8 py-4 font-bold text-sm transition-colors border-b-2',
          activeTab === 'absences'
...
function StudentAbsencesList({ absences }: { absences: Absence[] }) {
  if (!absences.length) {
    return (
      <EmptyState
        title="Sin inasistencias"
        description="El estudiante no registra faltas a la fecha."
      />
    )
  }
  ...
  return (
    <div className="space-y-4">
      {absences.map((abs) => (
        <div
          key={abs.id}
          className="p-4 bg-white border border-slate-100 rounded-2xl shadow-sm flex items-center justify-between hover:border-blue-100 transition-colors"
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-xs text-slate-500 mt-0.5">
                {abs.observation || 'Sin observación'}
              </div>
            </div>
          </div>
...
function StudentRecordsList({ records }: { records: InspectorateRecord[] }) {
  if (!records.length) {
    return (
      <EmptyState
        title="Sin atenciones"
        description="No hay registros de inspectoría para este alumno."
      />
    )
  }
  ...
  return (
    <div className="space-y-4">
      {records.map((rec) => (
        <div
          key={rec.id}
          className="p-5 bg-white border border-slate-100 rounded-2xl shadow-sm hover:border-blue-100 transition-colors"
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <Badge variant="secondary">
              {new Date(rec.date_time).toLocaleTimeString('es-CL', {
                hour: '2-digit',
                minute: '2-digit',
              })}
            </Badge>
