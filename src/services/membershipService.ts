...
export async function getMyMembership(applicationCode: string): Promise<MembershipResult> {
  const enabled = import.meta.env.VITE_APP_MEMBERSHIPS_ENABLED === 'true';
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const { data, error } = await withTimeout(
        Promise.resolve(
          (supabase.rpc as unknown as (name: string) => ReturnType<typeof supabase.rpc>)(
      ...
      const memberships = data as unknown as AppMembership[];
...
export function isMembershipsEnabled(): boolean {
  return import.meta.env.VITE_APP_MEMBERSHIPS_ENABLED === 'true';
}

export function getMembershipMode(): 'legacy' | 'transition' | 'enforced' {
  const enabled = import.meta.env.VITE_APP_MEMBERSHIPS_ENABLED === 'true';
  const enforced = import.meta.env.VITE_APP_MEMBERSHIPS_ENFORCED === 'true';
