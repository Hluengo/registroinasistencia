...
export const absenceService = {
  getAbsences: async (filters?: {
    ...
    const relevantCourseIds = Array.from(
      new Set(result.map((a) => a.students.course_id))
    )
    ...
    const tests = (allTests ||
      []) as Database['public']['Tables']['tests']['Row'][]
    const testsByCourse = tests.reduce(
      (acc, test) => {
  ...
  createAbsence: async (
    absence: Database['public']['Tables']['absences']['Insert'],
    file?: File
  ) => {
  ...
  updateAbsence: async (id: string, updates: Partial<Absence>, file?: File) => {
    ...
    const finalUpdates: Partial<
      Database['public']['Tables']['absences']['Update']
    > = { ...updates }
    if (document_url) {
