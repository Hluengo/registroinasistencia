...
export const studentService = {
  ...
  > => {
    try {
      const [studentRes, absencesRes, recordsRes] = await Promise.all([
        supabase
          .from('students')
          .select('id, full_name, course_id, rut, courses(id, name, level)')
          .eq('id', studentId)
          .single(),
        supabase
          .from('absences')
          .select('id, start_date, end_date, observation, status')
          .eq('student_id', studentId)
          .order('start_date', { ascending: false }),
        supabase
          .from('inspectorate_records')
          .select('id, date_time, observation')
          .eq('student_id', studentId)
  ...
  bulkInsertStudents: async (
    students: { full_name: string; course_id: string; rut?: string }[]
  ): Promise<ServiceResult<Student[]>> => {
    try {
      const { data, error } = await supabase
        .from('students')
        .insert(
          students as Database['public']['Tables']['students']['Insert'][]
        )
        .select()
      if (error) throw error
      return successResult(data as unknown as Student[])
