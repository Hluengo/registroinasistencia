...
import type { MembershipStatus, MembershipAuthMode, AppMembership } from '../types/membership';
import {
  getMyMembership,
  getMembershipMode,
...
export function useAuth() {
    ...
    async (userId?: string | null): Promise<AppRole> => {
      ...
      try {
        const profileRes = await withTimeout(
          Promise.resolve(
            supabase.from('profiles').select('role, tenant_id').eq('user_id', userId).maybeSingle()
          ),
          ROLE_TIMEOUT_MS
        );
  ...
  const loadMembership = React.useCallback(async (userId: string | null) => {
    ...
    try {
      ...
      setMembershipState(
        result.memberships.find((m) => m.application_code === 'inasistencias') ?? null
      );
      setMembershipError(result.status === 'error' ? 'Error al cargar membresía' : null);
      setMembershipLoaded(true);
  ...
  const membershipHasAccess =
    ...
    (membershipAuthMode === 'transition' && legacyFallbackUsed && membershipStatus !== 'inactive');
