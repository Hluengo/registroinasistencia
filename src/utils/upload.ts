...
function extractPublicUrl(response: unknown): string | null {
  ...
  const publicUrl =
    (data as Record<string, unknown>).publicUrl ||
    (data as Record<string, unknown>).publicURL
...
export const uploadFile = async (
  ...
  while (attempt < RETRY_CONFIG.MAX_ATTEMPTS) {
    ...
    try {
      ...
      lastError =
        err instanceof Error ? err.message : 'Error desconocido en upload'
      console.warn(`uploadFile: attempt ${attempt} exception`, err)
    }

    if (attempt < RETRY_CONFIG.MAX_ATTEMPTS) {
      await new Promise((resolve) =>
        setTimeout(resolve, attempt * RETRY_CONFIG.BASE_DELAY_MS)
      )
    }
  }
