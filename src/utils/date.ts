...
const buildLocalDate = (year: number, month: number, day: number): Date =>
  new Date(year, month - 1, day, 12, 0, 0, 0)

export const toDateOnlyString = (
  dateLike: string | Date | undefined | null
): string => {
  if (!dateLike) return ''
...
export const formatDateOnlyLocale = (
  dateLike: string | Date | undefined | null,
  locale = 'es-CL'
) => {
